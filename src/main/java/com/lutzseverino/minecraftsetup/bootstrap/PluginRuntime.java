...
final class PluginRuntime implements AutoCloseable {
    ...
    static PluginRuntime start(JavaPlugin plugin, SetupPublisherSettings settings)
            throws Exception {
        validateIdentityConfiguration(settings);
        Clock clock = Clock.systemUTC();
        CachedManifestSnapshotProvider manifests = new CachedManifestSnapshotProvider(
                new ProtocolManifestReader(settings.manifestSource())
        );
        ManifestPublicationService publication = new ManifestPublicationService(
                manifests,
                manifests,
                new AtomicFileManifestPublisher(settings.publicationOutput())
        );
        ManifestSnapshot loaded = settings.publishOnStart()
                ? publication.publish()
                : publication.activateWithoutPublishing();
        ...
        ChallengeIssuer challengeIssuer = new ChallengeIssuer(
                challenges,
                new SecureChallengeCodeGenerator(),
                clock,
                settings.gate().challengeTtl()
        );
        SetupCompliancePolicy policy = new SetupCompliancePolicy(
                settings.gate().mode(),
                settings.gate().failurePolicy()
        );
        LoginGateService loginGate = new LoginGateService(policy, manifests, compliance, challengeIssuer);
        AttestationService attestations = new AttestationService(challenges, compliance, manifests, clock);

        LoopbackSetupHttpServer server = null;
        if (settings.http().enabled()) {
            server = new LoopbackSetupHttpServer(
