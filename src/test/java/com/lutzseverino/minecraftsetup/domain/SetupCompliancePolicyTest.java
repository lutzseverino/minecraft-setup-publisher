...
class SetupCompliancePolicyTest {
  ...
  void enforcementRequiresMissingOrOutdatedPlayersToSetUp() {
    ...
    assertEquals(GateOutcome.DENY_SETUP_REQUIRED, policy.decide(false, Optional.empty(), CURRENT));
    assertEquals(
        GateOutcome.DENY_SETUP_OUTDATED, policy.decide(false, Optional.of(record(OLD)), CURRENT));
    assertEquals(
        GateOutcome.ALLOW_CURRENT, policy.decide(false, Optional.of(record(CURRENT)), CURRENT));
  }

  @Test
  void operationalFailureDefaultsCanKeepServerAvailable() {
    SetupCompliancePolicy failOpen =
        new SetupCompliancePolicy(GateMode.ENFORCE, FailurePolicy.ALLOW);
    SetupCompliancePolicy failClosed =
        new SetupCompliancePolicy(GateMode.ENFORCE, FailurePolicy.DENY);
  ...
  void advisoryModeDistinguishesFirstSetupFromAnUpdate() {
    SetupCompliancePolicy policy =
        new SetupCompliancePolicy(GateMode.ADVISORY, FailurePolicy.ALLOW);

    assertEquals(
        GateOutcome.ALLOW_ADVISORY_REQUIRED, policy.decide(false, Optional.empty(), CURRENT));
    assertEquals(
  ...
  private static ComplianceRecord record(ManifestFingerprint fingerprint) {
    return new ComplianceRecord(
        UUID.randomUUID(), fingerprint, new ProfileId("standard"), Instant.EPOCH);
  }
