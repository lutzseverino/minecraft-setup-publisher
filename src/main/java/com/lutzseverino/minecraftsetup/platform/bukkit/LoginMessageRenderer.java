...
final class LoginMessageRenderer {
  ...
  static Component render(
      LoginGateDecision decision, SetupPublisherSettings.MessageSettings messages) {
    ...
    String template =
        switch (decision.outcome()) {
          ...
          default ->
              throw new IllegalArgumentException("Login decision has no player-facing message");
        };
  ...
  static Component renderTemplate(String template, String code) {
    ...
    String parsedTemplate =
        template.replace(CONFIGURED_CODE_PLACEHOLDER, MINIMESSAGE_CODE_PLACEHOLDER);
    return MINI_MESSAGE.deserialize(
        parsedTemplate, Placeholder.component(MINIMESSAGE_CODE_TAG, Component.text(code)));
  }
}
