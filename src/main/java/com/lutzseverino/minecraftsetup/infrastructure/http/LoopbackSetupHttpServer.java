...
public final class LoopbackSetupHttpServer implements AutoCloseable {
        ...
        try (exchange) {
            ...
            if (result.acceptedFingerprint().isPresent()) {
                sendJson(exchange, 200, Map.of(
                        "status", "accepted",
                        "manifestFingerprint", result.acceptedFingerprint().orElseThrow().value()
                ));
                return;
            }
            ...
            switch (error) {
                case CHALLENGE_INVALID -> sendProblem(exchange, 404, "challenge_invalid", "Setup code is not valid");
                case CHALLENGE_EXPIRED -> sendProblem(exchange, 410, "challenge_expired", "Setup code expired");
                case FINGERPRINT_MISMATCH -> sendProblem(exchange, 409, "fingerprint_mismatch", "Server setup changed");
                case PROFILE_INVALID -> sendProblem(exchange, 409, "profile_invalid", "Setup profile is not valid");
                case ATTESTATION_UNAVAILABLE -> sendProblem(exchange, 503, "attestation_unavailable", "Setup check is unavailable");
            }
        }
    }
    ...
    private static void sendProblem(HttpExchange exchange, int status, String code, String title)
        ...
        send(exchange, status, JSON.writeValueAsBytes(Map.of(
                "type", "about:blank",
                "title", title,
                "status", status,
                "code", code
    ...
    private String clientKey(HttpExchange exchange) {
        ...
        if (candidate.length() < 2 || candidate.length() > 45
