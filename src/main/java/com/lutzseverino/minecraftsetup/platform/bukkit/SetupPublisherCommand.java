...
public final class SetupPublisherCommand implements CommandExecutor, TabCompleter {
    private static final List<String> SUBCOMMANDS = List.of("status", "validate", "publish", "reload");
    ...
    public boolean onCommand(CommandSender sender, Command command, String label, String[] arguments) {
        ...
        try {
            ...
            sender.sendMessage("Setup Publisher could not complete that action: " + exception.getMessage());
    ...
    private void showStatus(CommandSender sender) throws Exception {
        ...
        sender.sendMessage("Gate mode: " + plugin.settings().gate().mode().name().toLowerCase(Locale.ROOT));
        sender.sendMessage("HTTP adapter: " + (plugin.settings().http().enabled() ? "enabled" : "disabled"));
    }
